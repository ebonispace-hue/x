...
// Inisialisasi Firebase (versi compat - cocok dengan index.html)
let db = null;
let storage = null;
...
function showDashboard() {
  ...
  if (firebaseReady) {
    ...
    document.getElementById("totalAC").textContent = "Config Firebase";
    document.getElementById("totalB").textContent = "error";
    document.getElementById("totalAll").textContent = "Cek console";
    document.getElementById("latestHistory").innerHTML =
      '<p class="empty">Firebase error. Buka console (F12)</p>';
    document.getElementById("topPenyewa").innerHTML =
      '<p class="empty">Firebase error</p>';
  }
}
...
rentalForm.addEventListener("submit", async (e) => {
  ...
  if (!firebaseReady) {
    alert("Firebase belum siap. Cek console (F12).");
    return;
  }
  ...
  unsubscribe = db.collection("rentals")
    ...
    .onSnapshot(
      ...
      (error) => {
        ...
        document.getElementById("totalAC").textContent = "Error";
        document.getElementById("totalB").textContent = "Error";
        document.getElementById("totalAll").textContent = "Cek Rules Firestore";
      }
    );
}
      ...
      .map((r) => {
        const actionsHtml = masterActions
          ? '<div class="item-actions">' +
              '<button class="btn-action btn-edit" title="Edit" data-id="' + r.id + '"><i class="fas fa-pen"></i></button>' +
              '<button class="btn-action btn-delete" title="Hapus" data-id="' + r.id + '"><i class="fas fa-trash"></i></button>' +
            '</div>'
          : "";
...
editForm.addEventListener("submit", async (e) => {
  e.preventDefault();
  if (!isMaster() || !firebaseReady) return;
